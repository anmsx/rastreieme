class TrackingNumber
  ...
  before_save :ensure_object_exists_and_set_initial_info
  ...
  validates_inclusion_of :status, :in => [OPEN, DELAYED, FINISHED, UNKNOWN, RETURNED]
  ...
  REMOTE_STATUS = {
    ...
    'entrega efetuada'                      => TrackingNumber::FINISHED,
    'destinatário ausente'                  => TrackingNumber::UNKNOWN,
    'cancelada a última informação'         => TrackingNumber::UNKNOWN,
    'objeto perdido em assalto ao carteiro' => TrackingNumber::UNKNOWN,
    'não localizado no fluxo postal'        => TrackingNumber::UNKNOWN,
    'devolvido ao remetente'                => TrackingNumber::RETURNED,
    'saiu para entrega'                     => TrackingNumber::OPEN
  }
  ...
  ACRONYM = {
    ...
    'PG' => '(TIPO DESCONHECIDO)'
  }

  def number=(n_number)
    init_string = n_number.slice(0..1).upcase
    n = TrackingNumber::ACRONYM[init_string]
    if n.nil?
      errors.add(:number, "Tipo de objeto desconhecido (#{init_string})")
    else
      write_attribute :number, n_number
      write_attribute :kind, n
    end
  end
  ...
  def update_remote_status(status)
    ...
    raise "Invalid status #{status.situacao.downcase}" if n_status.nil?
  ...
  private
    def ensure_object_exists_and_set_initial_info
      _object = Correios.encomenda(number)
      if _object.status.count <= 0
        errors.add(:number, 'Objeto não existe.')
      else
        write_attribute :post_date, _object.primeiro_status_disponivel.data 
      end
    end
end
